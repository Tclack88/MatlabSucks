...
% Test 17: rAH_3 (Pretest)
% Find the position vector from point A to the center of mass of the rectangular hole in frame {3}
dAGhy = d1 + t3 + t1 -c/2;
rAH_3 = [0; -dAGhy; d5];
...
mh = a*b*c*dens1;
R23 = R32';
rAC1_2 = (rAD1_2*m1 + rAD3_2*m3 - R23*rAH_3*mh)/(m1+m3-mh);
...
Ih_3 = [Ixh 0 0;
        0 Iyh 0;
        0  0 Izh];
r_CH3_3 = rAH_3 - R32*(rAC1_2);
dx = [1 0 0] * r_CH3_3;
dy = [0 1 0] * r_CH3_3;
dz = [0 0 1] *  r_CH3_3;
IH_prime  = mh*[dy^2+dz^2    -dx*dy    -dx*dz;
           -dx*dy    dx^2+dz^2    -dy*dz;
           -dx*dz     -dy*dz    dx^2+dy^2];
I_rotorh = Ih_3 + IH_prime
%%% Here goes fucking nothing. Fingers crossed...
I_rotor_C1_3 = I_rotor1 + I_rotor3 - I_rotorh
